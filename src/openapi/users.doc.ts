import { responseUserSchema, selectUserSchema } from '@/db/users.schema';
import {
  queryUsersSchema,
  updateProfileSchema,
  updateUserSchema,
    ...
    get: {
      ...
      responses: {
        200: {
          ...
          content: { 'application/json': { schema: z.object({ user: responseUserSchema }) } }
        }
      }
    },
    put: {
      ...
      requestParams: { path: z.object({ id: z.string().openapi({ description: 'User id' }) }) },
      requestBody: {
      ...
      responses: {
        ...
        401: { description: 'User is not authrized' },
